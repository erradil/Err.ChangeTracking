...
Use `TryGetChangeTracker()` when you're unsure if tracking is initialized:

```csharp
var tracker = order.TryGetChangeTracker();
if (tracker == null)
{
    Console.WriteLine("Change tracking not initialized");
    return;
}

// Now safely use the tracker
if (tracker.IsDirty)
{
    // ...
}
...
## ⚡ Instance Storage vs Cache-Based
...
---
#### Instance Storage (High Performance) 
